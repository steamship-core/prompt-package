...
export function hasExample(name: string): Promise<boolean> {
  return isUrlOk(
    `https://api.github.com/repos/nludb/nludb-examples/contents/examples/${encodeURIComponent(
      name
    )}/package.json`
  )
}
...
export function downloadAndExtractExample(
  ...
  return pipeline(
    got.stream('https://codeload.github.com/nludb/nludb-examples/tar.gz/main'),
    tar.extract({ cwd: root, strip: 3 }, [`nludb-examples-main/examples/${name}`])
  )
}
