...
export async function helloWorld(client: Client): Promise<[App, AppVersion, AppInstance]> {
  ...
  const instance1r = (await AppInstance.create(client, { appId: app1.id!, appVersionId: version1.id!, config: { "greeting": "Hola" } }))
...
describe("ConfigurableApp", () => {
  ...
  test('it should be invocable', async () => {
    ...
    const instance2r = (await AppInstance.create(steamship, { appId: app1.id!, appVersionId: version1.id!, config: { "greeting": "Hallo" } }))
